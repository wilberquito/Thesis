...
There are three subcategories (see Figure \ref{fig:ml_branches}) of
...
Deep learning, a subset of machine learning algorithms (see Figure
...
Deep neural networks, also known as fully connected layers (see Figure
...
resemblance to human neurons (see Figure \ref{fig:perceptron}). The perceptron was
...
A convolutional layer (see Figure \ref{fig:convolutional-layer}) is
...
\begin{figure}[H] \centering
  ...
  \end{adjustbox}
  \caption[Convolutional operation]{\textit{Convolutional
...
A pooling layer (see Figure \ref{fig:pooling-layer}) performs a
...
\begin{figure}[H]
  ...
  \caption[Max Polling operation]{\textit{Max Polling operation on input. Illustration by towardsdatascience}}
...
A fully connected layer (see Figure \ref{fig:deep-nn}), helps to map the
...
A confusion matrix (see Figure \ref{fig:confusion-matrix}) is a square matrix with dimensions  NxN,
...
the negative class in separate binary classification tasks (see Figure
\ref{fig:auc-roc}). \\
...
Using the Gradient Decent (SD) optimization algorithm (see Figure
...
Resuming the forward-propagation and back-propagation (see Figure
...
There are lot of strategies to combat over-fitting, in this section we present
the main thecniques used in the thesis.
...
\begin{itemize}
  \item \textbf{Learning Rate Decay}
    ...
    learning rate decay over the epochs. For instance; Cosine Annealing Learning Rate (see Figure
    \ref{fig:learning-rate-decay-cosine-annealing}) and Step Learning Rate (see Figure
    \ref{fig:learning-rate-decay-step}).
  ...
  \item \textbf{Cycling Learning}
    ...
    own policy. One of the most popular cycling learning scheduler is Cosine
    Annealing Warm Restart (see Figure \ref{fig:cycling-rate-decay}).
...
original dataset into three subsets using the Holdout set scheme (see Figure

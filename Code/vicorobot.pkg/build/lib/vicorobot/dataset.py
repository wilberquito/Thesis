...
def get_df(kernel_type, out_dim, data_dir, data_folder, use_meta):
    ...
    #print("break point: len df train", len(df_train))
    """this part was for isic data tfrecords - sana
    ...
    #print("break point:len df train 2", len(df_train2))
    """this paert was for isic data tfrecords - sana
    if 'newfold' in kernel_type:
    ...
    if out_dim == 8: #sana changed it from 9 to 8
        ...
        #print("break point: df_train2['diagnosis']", len(df_train2['diagnosis']))
    elif out_dim == 4:
        df_train2['diagnosis'] = df_train2['diagnosis'].apply(lambda x: x.replace('NV', 'nevus'))
        df_train2['diagnosis'] = df_train2['diagnosis'].apply(lambda x: x.replace('MEL', 'melanoma'))
    ...
    if use_meta:
        ...
        meta_features = None
        n_meta_features = 0
    #print("break point:len final df test",len (df_test))
    # class mapping
    diagnosis2idx = {d: idx for idx, d in enumerate(sorted(df_train.diagnosis.unique()))}
    df_train['target'] = df_train['diagnosis'].map(diagnosis2idx)
    #print("diagnosis2idx",diagnosis2idx)
    #print("classes: " ,set(df_train['target'] ))
